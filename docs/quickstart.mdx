...
The installer detects your platform (`uname -s/-m`), pulls the matching wheel from the latest release on `beava-dev/beava`, and `pip install --user`'s it. `--memory-only` skips the WAL for a kick-the-tyres run — drop it for production (state lands under `./.beava/` by default). Pin a specific version with `BEAVA_VERSION=v0.4.0 curl ... | sh`.
