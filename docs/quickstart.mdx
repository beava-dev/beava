...
The installer detects your platform (`uname -s/-m`), pulls the matching wheel from the latest release on `beava-dev/beava`, and `pip install --user`'s it. `--memory-only` skips the WAL for a kick-the-tyres run — drop it for production (state lands under `./.beava/` by default). Pin a specific version with `BEAVA_VERSION=v0.0.0 curl ... | sh`.
...
# the SDK still installs via curl|sh; pass an explicit URL to bv.App(url=...)
curl -fsSL https://raw.githubusercontent.com/beava-dev/beava/main/scripts/install.sh | sh
...
<Note>
PyPI publish lands at v0.0.x — `pip install beava` will work then. For now the wheels ship from GitHub Releases and `curl | sh` is the canonical install path. Both surfaces will coexist after the PyPI cut.
</Note>
