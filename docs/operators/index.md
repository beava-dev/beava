...
> 56 operator pages (55 unique AggKind variants + `ema` alias documented inline in `ewma.md`), across 7 family subdirectories.
...
- `bv.ema` is an alias of [`bv.ewma`](./decay/ewma.md) — documented inline on the ewma page (same `AggKind::Ewma` variant; 55 unique kinds + 1 alias = 56 page paths).
