...
- **Server processing-time only.** Decay coefficients use `Δt = now_ms() at this matching event - now_ms() at the previous matching event` per [`project_redis_shaped_no_event_time_ever`](../../../.planning/PROJECT.md). Producers cannot influence decay via payload fields. Late events (`Δt ≤ 0`) fall back to an unweighted blend and do **not** advance `last_now_ms`.
