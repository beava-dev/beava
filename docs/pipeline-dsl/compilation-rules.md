...
**Predicate nesting depth is bounded at 512.** The parser enforces a maximum
nesting depth of 512 (`MAX_PARSE_DEPTH`) — matching the runtime evaluator's
recursion cap (`MAX_EVAL_DEPTH`). Predicates exceeding this depth (e.g., a
600-deep `&` chain) are rejected at register time with `RegistrationError`
carrying code `invalid_expression` (for filter ops) or
`aggregation_invalid_where` (for aggregation `where=` clauses). The error
message cites "depth" and the limit value (512). This is a compile-time
safeguard: before this guard, overly-nested predicates passed registration
cleanly but then silently returned Null at evaluation time, filtering out all
events with no diagnostic.
...
| `event_time_field=` / `tolerate_delay=` kwargs on `@bv.event` | FORBIDDEN per same lock (post-2026-05-19: partially overturned by ADR-004 for v0.1 bucketing) | Same as above. | `TypeError` at decoration time |
