...
- **Nesting depth exceeds limit** — predicates with nesting depth exceeding 512
  are rejected at parse time with `RegistrationError(code="aggregation_invalid_where")`
  for aggregation `where=` clauses or `code="invalid_expression"` for filter ops.
  The error message will mention "depth" and cite the limit (512). This
  prevents the previous silent behavior where over-deep predicates would pass
  registration but then silently filter out every event at evaluation time. The
  limit of 512 mirrors the runtime evaluation depth limit (`MAX_EVAL_DEPTH`) to
  ensure any expression that parses successfully is also safe to evaluate.
