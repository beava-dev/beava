...
sync-mode trade-offs, and what got reset in the v0 schema strip.
...
an internal plan RESET both `FORMAT_VERSION` (WAL) and
...
before booting the new binary. (Locked decision per v0 D-01:
...
| `0x03` | (deleted) | Was `TableUpsert`; pre-v0 only — surfaces as `UnknownRecordType` on recovery |
| `0x04` | (deleted) | Was `TableDelete`; pre-v0 only |
| `0x05` | (deleted) | Was `Retract`; pre-v0 only |

Bytes `0x03` / `0x04` / `0x05` fall through to the existing
`UnknownRecordType` arm — pre-v0 dev WALs that carried these bytes
surface the standard "unknown record_type" error on recovery, which is
the operator's signal to clear the WAL.

## Snapshot format (v0 RESET)
...
The WAL writer has two sync modes:
...
`OP_PUSH_SYNC` is wired through v0 but the SDK opcode is v0.1+.
See [`.planning/ideas/v0.1-deferrals.md`](../../.planning/ideas/v0.1-deferrals.md).

## Four-watermark LSN discipline

The WAL writer maintains four watermarks per v0 design (memory
...
writer is fsyncing. Buffer size: 16 MiB × 3 buffers (v0 lock).
...
- **Pre-12.7 WALs / snapshots are not readable by v0 binaries.**
  Clear `<persist_dir>/wal` + `<persist_dir>/snapshots` before
  upgrading from a pre-v0 dev binary. There is no migration tool.

## Cross-references

- [`CLAUDE.md` § Events-Only Invariant (locked v0)](../../CLAUDE.md)
...
- `~/.claude/projects/-Users-petrpan26-work-tally/memory/project_phase18_wal_architecture.md`
  — locked v0 WAL architecture (lock-free apply, 3-buffer state
