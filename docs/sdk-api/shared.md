...
- [TypeScript SDK](typescript.md) — npm packages `@beava/node` (Node.js) and `@beava/client` (browser); both export the same communicate-only API (no DSL).
...
- TypeScript: `createBeavaClient({ baseUrl })` with explicit URL (no embed mode in v0 — HTTP only).
...
| TypeScript | `await beava.get({ table: "Table", key: "alice" })` | `await beava.get({ table: "Table", key: "" })` |
...
- **TypeScript** uses explicit `key: ""` in the object parameter — consistent with the single-object-parameter design.
...
- TypeScript: object parameter with snake_case keys — `beava.register({ nodes, force: false, dry_run: false })`.
...
|-------------|--------|------------|-----|
| (constructor) | `bv.App()` or `bv.App(url)` | `createBeavaClient({ baseUrl, ... })` | `beava.NewApp(ctx, url)` |
| `OP_REGISTER` | `app.register(*descriptors, force=False, dry_run=False)` | `beava.register({ nodes, force, dry_run })` | `app.Register(ctx, descriptors, beava.WithForce(), beava.WithDryRun())` |
| `OP_PUSH` | `app.push(event_name, fields)` | `beava.push({ event, data })` | `app.Push(ctx, eventName, fields)` |
| `OP_PUSH_SYNC` (reserved) | `app.push_sync(event_name, fields)` | (not in v0) | `app.PushSync(ctx, eventName, fields)` |
| `OP_GET` (per-entity) | `app.get(table, key)` | `beava.get({ table, key })` | `app.Get(ctx, table, key)` |
| `OP_GET` (global, ADR-003) | `app.get(table)` (1-arg) | `beava.get({ table, key: "" })` | `app.GetGlobal(ctx, table)` (separate method) |
| `OP_BATCH_GET` | `app.batch_get(requests)` | `beava.batchGet({ requests })` | `app.BatchGet(ctx, requests)` |
| `OP_RESET` | `app.reset()` | `beava.reset()` | `app.Reset(ctx)` |
| `OP_PING` | `app.ping()` | `beava.ping()` | `app.Ping(ctx)` |
| (close lifecycle) | `app.close()` / context manager | (no close in v0) | `app.Close(ctx)` / `defer` |
