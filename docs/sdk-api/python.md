...
> **Module name:** `pip install beava` (published to PyPI as of v0.4.0; the wheel ships both the Python SDK and the Rust server binary). Import as `import beava as bv`.
...
**Binary discovery** (embed mode only) follows a three-step order:

1. `$BEAVA_BINARY` environment variable (if set) — direct path to an executable; raises `BinaryNotFoundError` if the path is invalid. No fallback.
2. `<sysconfig.get_path("scripts")>/beava` — the maturin-bundled binary shipped with the wheel from v0.4.0 onward. Checked before `PATH` so embed mode always uses the binary matching the active Python's installed SDK version, avoiding version skew with stale system-wide binaries.
3. `beava` on `PATH` via `shutil.which` (shebang scripts excluded — older 0.3.x wheels installed a Python console-script shim; execing into it would loop).
4. Walk CWD upward for `target/release/beava` or `target/debug/beava` (dev-loop convenience for in-tree work).
5. Raise `BinaryNotFoundError`.

The discovery order (per `python/beava/_embed.py`) ensures `pip install beava` provides the server binary without extra install steps.
...
class BinaryNotFoundError(Exception):
    pass                               # raised by embed mode when binary not found via the discovery order
